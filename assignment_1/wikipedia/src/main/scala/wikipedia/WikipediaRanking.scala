...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setAppName("wikipedia-lang").setMaster("local[4]")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd.aggregate(0)((i, article) => if (article.mentionsLanguage(lang)) i + 1 else i, (i, j) => i + j)
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.mapValues(v => v.size).collect().sortBy(_._2)(Ordering[Int].reverse).toList
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    ...
    }).reduceByKey(_ + _).collect().sortBy(_._2)(Ordering[Int].reverse).toList
  }
